...
{
    public class DiadocSyncContractService : IScheduledTask
    {
        private readonly IServiceScopeFactory _serviceScopeFactory;
        private readonly IConfiguration _configuration;

        public string ServiceName { get => "DiadocSyncContractService"; }

        public DiadocSyncContractService(IServiceScopeFactory serviceScopeFactory, IConfiguration configuration)
        {
                    ...
                    do
                    {
                        ...
                        foreach (DiadocSyncContractWorker worker in AllWorkers)
                        {
                            ...
                            catch (Exception e)
                            {
                                ...
                                _timerJobLogService.AddError("Произощла ошибка во время синхронизации документов по подключению " + worker.Settings.EdiSettings.ConnectionInfo.OrganizationName + ": " + e.Message, ServiceName, worker.Settings.EdiSettings.ConnectionInfo.ClientID);
                            }
                        }
        ...
        private async Task<List<DiadocSyncContractWorker>> GetWorkers(SearchServiceDBContext _dbContext, IBackgroundServiceLog _timerJobLogService, List<DiadocSyncContractWorker> AllWorkers, IAdminService _db, ICommonService _commonService, IStorageService<StoredFile> _fileStorage)
        {
            if (AllWorkers == null || AllWorkers.Count == 0)
            {
                ...
                AllWorkers = new List<DiadocSyncContractWorker>();
                ...
                foreach (EDISettings set in Settings)
                {
                    ...
                    if (checkQuota.res)
                    {
                        ...
                        DiadocSyncContractWorker wrk = new DiadocSyncContractWorker(settings);
                        AllWorkers.Add(wrk);
                    }
                    else
                    {
